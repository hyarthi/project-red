...
public class WrapperFactory extends BaseImpl<lotus.domino.Base> implements org.openntf.domino.WrapperFactory {
	...
	T fromCouch(final D couch, final FactorySchema<T, D, P> schema, P parent, final Collection<lotus.domino.Base> prevent_recycling,
		...
		if (couch instanceof Base) {
			if (log_.isLoggable(Level.FINE))
				log_.log(Level.FINE, "Returning an already OpenNTF object...");
			return (T) couch;
		}

		if (!(couch instanceof CouchBase)) {
			throw new UndefinedDelegateTypeException("Cannot wrap " + couch.getClass().getName());
		}

		if (parent == null) {
			try {
				parent = (P) findParent(couch);
			} catch (NotesException e) {
				DominoUtils.handleException(e);
			}
		}
		if (parent == null)
			throw new UndefinedDelegateTypeException("Cannot find parent for a " + couch.getClass().getName());
		/*
	...
	protected Base<?> wrapLotusObject(final lotus.domino.Base lotus, final Base<?> parent) {
		...
		if (lotus instanceof lotus.domino.Name) {
			if (((Session) parent).isFixEnabled(Fixes.ODA_NAMES))
				return new org.openntf.redomino.impl.NameODA((lotus.domino.Name) lotus, (Session) parent);
			return new org.openntf.domino.impl.Name((lotus.domino.Name) lotus, (Session) parent);
		}

		if (lotus instanceof lotus.domino.DateRange) {
			return new org.openntf.redomino.impl.DateRange((lotus.domino.DateRange) lotus, (Session) parent);
		}

		if (lotus instanceof lotus.domino.DateTime) {
			return new org.openntf.redomino.impl.DateTime((lotus.domino.DateTime) lotus, (Session) parent);
		}
		...
		if (lotus instanceof lotus.domino.MIMEEntity) {
			return new org.openntf.redomino.impl.MIMEEntity((lotus.domino.MIMEEntity) lotus, (Document) parent);
		}

		if (lotus instanceof lotus.domino.MIMEHeader) {
			return new org.openntf.redomino.impl.MIMEHeader((lotus.domino.MIMEHeader) lotus, (MIMEEntity) parent);
		}
		...
		if (lotus instanceof lotus.domino.AgentContext) {
			return new org.openntf.redomino.impl.AgentContext((lotus.domino.AgentContext) lotus, (Session) parent);
		}
		if (lotus instanceof lotus.domino.ColorObject) {
			return new org.openntf.redomino.impl.ColorObject((lotus.domino.ColorObject) lotus, (Session) parent);
		}
		if (lotus instanceof lotus.domino.Database) {
			return new org.openntf.redomino.impl.Database((lotus.domino.Database) lotus, (Session) parent);
		}
		if (lotus instanceof lotus.domino.DbDirectory) {
			return new org.openntf.redomino.impl.DbDirectory((lotus.domino.DbDirectory) lotus, (Session) parent);
		}
		...
		if (lotus instanceof lotus.domino.DxlImporter) {
			return new org.openntf.redomino.impl.DxlImporter((lotus.domino.DxlImporter) lotus, (Session) parent);
		}
		if (lotus instanceof lotus.domino.EmbeddedObject) {
			return new org.openntf.redomino.impl.EmbeddedObject((lotus.domino.EmbeddedObject) lotus, (Document) parent);
		}
		if (lotus instanceof lotus.domino.Form) {
			return new org.openntf.redomino.impl.Form((lotus.domino.Form) lotus, (Database) parent);
		}
		if (lotus instanceof lotus.domino.International) {
			return new org.openntf.redomino.impl.International((lotus.domino.International) lotus, (Session) parent);
		}
		if (lotus instanceof lotus.domino.Log) {
			return new org.openntf.redomino.impl.Log((lotus.domino.Log) lotus, (Session) parent);
		}
		...
		if (lotus instanceof lotus.domino.OutlineEntry) {
			return new org.openntf.redomino.impl.OutlineEntry((lotus.domino.OutlineEntry) lotus, (Outline) parent);
		}
		if (lotus instanceof lotus.domino.PropertyBroker) {
			return new org.openntf.redomino.impl.PropertyBroker((lotus.domino.PropertyBroker) lotus, (Session) parent);
		}
		if (lotus instanceof lotus.domino.Registration) {
			return new org.openntf.redomino.impl.Registration((lotus.domino.Registration) lotus, (Session) parent);
		}
		...
		if (lotus instanceof lotus.domino.Session) {
			return new org.openntf.redomino.impl.Session((lotus.domino.Session) lotus, this);
		}
	...
	protected Base<?> findParent(final lotus.domino.Base lotus) throws NotesException {
		// TODO inject all Couch objects into this function
		if (lotus instanceof lotus.domino.Name) {
		...
		if (lotus instanceof lotus.domino.Database) {
			// Couch extension
			if (lotus instanceof CouchBase)
				return fromCouch(((lotus.domino.Database) lotus).getParent(), Session.SCHEMA, null);
	...
	public <T extends Base, P extends Base> T create(final FactorySchema<T, ?, P> schema, final P parent, final Object metaData) {
		...
		if (schema == Name.SCHEMA) {
			...
			if (((Session) parent).isFixEnabled(Fixes.ODA_NAMES))
				return (T) new org.openntf.redomino.impl.NameODA((Session) parent, name, lang);
			return (T) ((Session) parent).createNameNonODA(name);
		}

		if (schema == Document.SCHEMA) {
			return (T) new org.openntf.redomino.impl.Document((Database) parent, metaData); // noteID/UNID
		}
	...
	public void recycle(final Vector vec) {
		org.openntf.domino.impl.ODAPincher.s_recycle(vec);
	}

	@Override
	public void recycle(final lotus.domino.Base obj) {
		org.openntf.domino.impl.ODAPincher.s_recycle(obj);
	}
