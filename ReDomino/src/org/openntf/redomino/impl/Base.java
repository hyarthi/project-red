...
public abstract class Base<T extends org.openntf.domino.Base<D>, D extends lotus.domino.Base, P extends org.openntf.domino.Base<?>>
	...
	protected final CouchBase beObject;
		...
		try {
			AccessController.doPrivileged(new PrivilegedExceptionAction<Object>() {
				...
				public Object run() throws Exception {
					...
					checkObjectMethod = lotus.domino.local.NotesBase.class.getDeclaredMethod("CheckObject",
							(Class<?>[]) null);
					checkObjectMethod.setAccessible(true);
					checkObjectActiveMethod = lotus.domino.local.NotesBase.class.getDeclaredMethod("CheckObjectActive",
							(Class<?>[]) null);
					checkObjectActiveMethod.setAccessible(true);
					clearCppObjMethod = lotus.domino.local.NotesBase.class.getDeclaredMethod("ClearCppObj",
							(Class<?>[]) null);
					clearCppObjMethod.setAccessible(true);
					getCppObjMethod = lotus.domino.local.NotesBase.class.getDeclaredMethod("GetCppObj",
							(Class<?>[]) null);
					getCppObjMethod.setAccessible(true);
					getCppSessionMethod = lotus.domino.local.NotesBase.class.getDeclaredMethod("GetCppSession",
							(Class<?>[]) null);
					getCppSessionMethod.setAccessible(true);
					getGCParentMethod = lotus.domino.local.NotesBase.class.getDeclaredMethod("getGCParent",
							(Class<?>[]) null);
					getGCParentMethod.setAccessible(true);
					getSessionMethod = lotus.domino.local.NotesBase.class.getDeclaredMethod("getSession",
							(Class<?>[]) null);
					...
					isInvalidMethod = lotus.domino.local.NotesBase.class.getDeclaredMethod("isInvalid",
							(Class<?>[]) null);
					...
					markInvalidMethod = lotus.domino.local.NotesBase.class.getDeclaredMethod("markInvalid",
							(Class<?>[]) null);
					markInvalidMethod.setAccessible(true);
					validateObjArgMethod = lotus.domino.local.NotesBase.class.getDeclaredMethod("validateObjArg",
	...
	protected P findParent(final WrapperFactory wf, final D delegate, final int i) throws NotesException {
		throw new UnsupportedOperationException(
				"You must specify a valid parent when creating a " + getClass().getName());
	}
	...
	// System.out.println("Jump greater than 100000 when we wrapped object count
	// " + traceWrapCount.get());
	// }
	// super.set(value);
	...
	protected Base(final D delegate, final P parent, final int classId) {
		...
		} else if (delegate instanceof org.openntf.redomino.couch.CouchBase) {
			// anything to initialize?
			beObject = (CouchBase) delegate;
			// TODO change this stub
			setDelegate(delegate, false);
		} else if (delegate != null) {
			// normally you won't get here if you come from fromLotus
			throw new IllegalArgumentException("Why are you wrapping this object? " + delegate.getClass().getName());
		} else
			beObject = null;
	}
	...
	/**
	 * Checks if the lotus object is invalid. A object is invalid if it is
	 * recycled by the java side.
	 * 
	...
	/**
	 * Checks if is dead. A object is dead if it is invalid (=recycled by java)
	 * or if it's cpp-object = 0.
	 * 
	...
	protected D getDelegate() {
		...
		if (this instanceof Resurrectable && isDead(ret)) {
			if (log_.isLoggable(Level.FINE))
				log_.fine("[" + Thread.currentThread().getId() + "] Resurrecting " + getClass().getName() + " '"
						+ super.hashCode() + "'");
			resurrect();
			if (log_.isLoggable(Level.FINE))
				log_.fine("[" + Thread.currentThread().getId() + "] Resurrect " + getClass().getName() + " '"
						+ super.hashCode() + "' done");
	 ...
	 *      FactorySchema, org.openntf.domino.Base)
	 */
	 ...
	 * 
	 * @see org.openntf.domino.WrapperFactory#fromLotus(Collection,
	 *      FactorySchema, org.openntf.domino.Base)
	 */
	@SuppressWarnings({ "rawtypes" })
	 ...
	 * 
	 * @see org.openntf.domino.WrapperFactory#fromLotusAsVector(Collection,
	 *      FactorySchema, org.openntf.domino.Base)
	 */
	@SuppressWarnings({ "rawtypes" })
	 ...
	 * Wraps column values
	 * 
	 * @see org.openntf.domino.WrapperFactory#wrapColumnValues(Collection,
	...
	/*
	 * (non-Javadoc) This method recycles the delegate (and counts it as manual
	 * recycle)
	 * 
	 * @see lotus.domino.Base#recycle() (it is NOT deprecated in the .impl.
	 * package as it is called in several places!)
	 */
	@Override
	public void recycle() {
	...
	protected static <T extends lotus.domino.Base> T toLotus(final T wrapper, final Collection recycleThis) {
		if (wrapper instanceof org.openntf.domino.impl.Base) {
			lotus.domino.Base ret = ((org.openntf.redomino.impl.Base) wrapper).getDelegate();
	...
	protected static <T extends lotus.domino.Base> T toLotus(final T wrapper) {
		if (wrapper instanceof org.openntf.domino.impl.Base) {
			return (T) ((org.openntf.redomino.impl.Base) wrapper).getDelegate();
		}
		return wrapper;
	}
	 ...
	 * @param baseObj
	 *            the base obj
	 * @return the lotus.domino. base version or the object itself, as
	 *         appropriate
	 */
	@SuppressWarnings("rawtypes")
	protected static Object toLotus(final Object baseObj) {
		if (baseObj instanceof org.openntf.domino.impl.Base) {
			return ((org.openntf.redomino.impl.Base) baseObj).getDelegate();
		}
		return baseObj;
	}
	...
	// *
	// * @deprecated use {@link #toDominoFriendly(Object,
	// org.openntf.domino.Base, Collection)} instead
	// * @param value
	// * @param context
	// * @return
	...
	// */
	// @Deprecated
	// protected static Object toDominoFriendly(final Object value, final
	// Session session) throws IllegalArgumentException {
	// return toDominoFriendly(value, session, null);
	// }

	/**
	 * toItemFriendly: special case for "toDominoFriendly" that handles
	 * "DateTime" / "DateRange" correctly
	 * 
	 * @param value
	 ...
	 * @throws IllegalArgumentException
	 *             When the provided value cannot be successfully converted into
	 *             an Item-safe value.
	 */
	protected static Object toItemFriendly(final Object value, final org.openntf.domino.Session session,
		...
		if (value instanceof lotus.domino.Base) {
			if (value instanceof lotus.domino.Name) {
				...
				// according to documentation, these datatypes should be
				// compatible to write to a field ... but DateRanges make
				// problems
			...
			throw new IllegalArgumentException(
					"Cannot convert to Domino friendly from type " + value.getClass().getName());
		} else {
			return javaToDominoFriendly(value, session, recycleThis);
		}
	}
	 ...
	 * 
	 * <p>
	 * Attempts to convert a provided scalar value to a "Domino-friendly" data
	 * type like DateTime, String, etc. Currently, the data types supported are
	 * the already-Domino-friendly ones, Number, Date, Calendar, and
	 * CharSequence.
	 * </p>
	 * 
	 * @param value
	 ...
	 * @return The Domino-friendly conversion of the object, or the object
	 *         itself if it is already usable.
	 * @throws IllegalArgumentException
	 *             When the object is not convertible.
	 */
	@SuppressWarnings("rawtypes")
	protected static Object toDominoFriendly(final Object value, final Session session,
			final Collection<lotus.domino.Base> recycleThis) throws IllegalArgumentException {
		if (value == null) {
			log_.log(Level.INFO, "Trying to convert a null argument to Domino friendly. Returning null...");
			return null;
	 ...
	 * @return
	 */
	private static Object javaToDominoFriendly(final Object value, final Session session,
		...
		if (value instanceof Integer || value instanceof Double) {
			...
			// FIXME: NTF IS A COMPLETE HACK!!!! (but we just want to know if
			// it'll fix PWithers' problem)
		...
		if (value instanceof Number) {
			// TODO Check if this is greater than what Domino can handle and
			// serialize if so
			// CHECKME: Is "doubleValue" really needed. (according to help.nsf
			// only Integer and Double is supported, so keep it)
			...
			try {
				...
				if (value instanceof java.util.Date) {
					...
					dt = lsess.createDateTime(
							sdf.format(intermediate.getTime()) + " " + intermediate.getTimeZone().getID());
				}
				if (recycleThis != null) {
					recycleThis.add(dt);
				}
			...
			// return
			// toLotus(Factory.getSession(context).createDateTime((java.util.Date)
			// value));
		} else if (value instanceof CharSequence) {
			return value.toString();
			// } else if (value instanceof CaseInsensitiveString) { //
			// CaseInsensitiveString is a CharSequence
			// return value.toString();
	...
	// * To domino friendly.
	// *
	// * @deprecated use {@link #toDominoFriendly(Collection,
	// org.openntf.domino.Base, Collection)}
	// * @param values
	// * the values
	// * @param context
	// * the context
	...
	// */
	// @Deprecated
	// protected static java.util.Vector<Object> toDominoFriendly(final
	// Collection<?> values, final Session session)
	// throws IllegalArgumentException {
	// java.util.Vector<Object> result = new java.util.Vector<Object>();
	...
	protected static boolean s_recycle(final lotus.domino.Base base) {
		...
		// if (!isLocked(base)) {
		try {
			if (base instanceof lotus.domino.local.DateRange) { // NTF - check
																// to see if we
																// have valid
																// start/end
																// dates to
		...
		// } else {
		// System.out.println("Not recycling a " + base.getClass().getName() + "
		// because it's locked.");
		// }
		return result;
	}
	...
	protected static void s_recycle(final Object o) {
		// NTF this is for recycling of encapsulated objects like DateTime and
		// Name
	...
	// if (!c.isEmpty()) {
	// for (Object io : c) {
	// if (io instanceof lotus.domino.DateTime || io instanceof
	// lotus.domino.DateRange || io instanceof lotus.domino.Name) {
	// s_recycle((lotus.domino.Base) io);
	// }
	// }
	// }
	// } else if (o instanceof lotus.domino.DateTime || o instanceof
	// lotus.domino.DateRange || o instanceof lotus.domino.Name) {
	// s_recycle((lotus.domino.Base) o);
	// }
	//
	// }
	...
	// these are important to link deferred against, so that they aren't
	// recycled.
	// defered objects that have the same delegate build a circle, so that they
	// aren't gc-ed as
	// long as someone is holding a reference to at least one object of this
	// circle.

	protected Base<?, ?, ?> siblingWrapper_;

	// The origin of the deferreds
	// RPR: Currently, if you use multiple deferreds pointing to the same
	// document, you MAY
	...
	// ------------ native functions - to be as much compatible to the
	// lotus.domino.local...
	...
	private static final int EXTERNALVERSIONUID = 20141205; // The current date
															// (when it was
															// implemented)
	...
	public void writeExternal(final ObjectOutput out) throws IOException {
		...
		// I also do not know if serializing listeners will make sense, so I
		// don't do it
	}
	...
	protected void readResolveCheck(final Object expected, final Object given) {
		if (expected == null ? given != null : !expected.equals(given)) {
			log_.warning("Deserializing different " + getClass().getSimpleName() + ". Given: " + given + ", expected: "
					+ expected);
		}
	}
	...
	protected void setNewSibling(final org.openntf.redomino.impl.Base<?, ?, ?> oldWrapper) {
