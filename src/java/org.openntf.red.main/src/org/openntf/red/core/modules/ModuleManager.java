...
public enum ModuleManager {
	;
	private static Logger log = Logger.getLogger(ModuleManager.class.getName());
	...
	public static void startServices() {
		...
		if (inst != null && (modifiers & Modifier.PUBLIC) > 0 && (modifiers & Modifier.STATIC) > 0 && (IDataBroker.class.isAssignableFrom(inst.getReturnType()))) {
			...
			log.finest("Module Manager: No getInstance() method found. Trying to instantiate via public Constructor().");
			// none present, check the class itself
			if (!IDataBroker.class.isAssignableFrom(classInst)) {
				log.severe("Module Manager: Exception - Class declared for Data Broker is incompatible with the runtime.");
				throw new ModuleManagerException("Incompatible object type for Data Broker.", new IllegalArgumentException("Can not convert from " + classInst.getName() + " to " + IDataBroker.class.getName() + "."));
			}
			// check Constructor()
			try {
			...
			if ((modifiers & Modifier.PUBLIC) > 0 && (modifiers & Modifier.ABSTRACT) == 0 && (cmodifiers & Modifier.ABSTRACT) == 0 && (cmodifiers & Modifier.INTERFACE) == 0 && (cmodifiers & Modifier.PUBLIC) > 0) {
		...
		modifiers = inst.getModifiers();
		if (inst != null && (modifiers & Modifier.PUBLIC) > 0 && (modifiers & Modifier.STATIC) > 0 && (ISecurityManager.class.isAssignableFrom(inst.getReturnType()))) {
			...
			try {
				...
				throw new ModuleManagerException("Module Manager: Failed to run instantiation static method for Security Manager.", ite);
			}
		} else {
			log.finest("Module Manager: No getInstance() method found. Trying to instantiate using a public Constructor().");
			// none present, check the class itself
			if (ISecurityManager.class.isAssignableFrom(classInst)) {
				log.severe("Module Manager: Exception - Class declared for Security Manager is not compatible with the runtime.");
				throw new ModuleManagerException("Module Manager: Incompatible object type for Security Manager.", new IllegalArgumentException("Can not convert from " + classInst.getName() + " to " + ISecurityManager.class.getName() + "."));
			}
			// check Constructor()
			try {
				constr = classInst.getConstructor();
			...
			if ((modifiers & Modifier.PUBLIC) > 0 && (modifiers & Modifier.ABSTRACT) == 0 && (cmodifiers & Modifier.ABSTRACT) == 0 && (cmodifiers & Modifier.INTERFACE) == 0 && (cmodifiers & Modifier.PUBLIC) > 0) {
				...
				try {
					...
					throw new ModuleManagerException("Module Manager: Failed to instantiate object for Security Manager.", e);
				}
			} else {
		...
		modifiers = inst.getModifiers();
		if (inst != null && (modifiers & Modifier.PUBLIC) > 0 && (modifiers & Modifier.STATIC) > 0 && (INetworkManager.class.isAssignableFrom(inst.getReturnType()))) {
			...
			try {
				...
				log.throwing(ModuleManager.class.getName(), "startService()", ite);
				throw new ModuleManagerException("Module Manager: Failed to run instantiation static method for Network Manager.", ite);
			}
		} else {
			log.finest("Module Manager: No getInstance() method found. Trying to instantiate using a public Constructor().");
			// none present, check the class itself
			if (INetworkManager.class.isAssignableFrom(classInst)) {
				log.severe("Module Manager: Exception - Class declared for Network Manager is not compatible with the runtime.");
				throw new ModuleManagerException("Module Manager: Incompatible object type for Network Manager.", new IllegalArgumentException("Can not convert from " + classInst.getName() + " to " + INetworkManager.class.getName() + "."));
			}
			// check Constructor()
			try {
				constr = classInst.getConstructor();
			...
			if ((modifiers & Modifier.PUBLIC) > 0 && (modifiers & Modifier.ABSTRACT) == 0 && (cmodifiers & Modifier.ABSTRACT) == 0 && (cmodifiers & Modifier.INTERFACE) == 0 && (cmodifiers & Modifier.PUBLIC) > 0) {
				...
				try {
					...
					log.throwing(ModuleManager.class.getName(), "startServices()", e);
					throw new ModuleManagerException("Module Manager: Failed to instantiate object for Network Manager.", e);
				}
			} else {
				log.severe("Module Manager: Could not find suitable constructor for Security Manager.");
