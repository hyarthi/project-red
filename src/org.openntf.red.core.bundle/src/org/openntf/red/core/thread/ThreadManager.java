...
public enum ThreadManager {
	...
	public static void registerDelayedTask(Runnable task, long period, TimeUnit unit) {
		// TODO code here
	}

	public static void registerScheduledTask(Runnable task, Date when) {
		// TODO code here
	}

	public static void registerRepeatingTask(Runnable task, long period, TimeUnit unit) {
		registerDelayedRepeatingTask(task, 0, period, unit);
	}

	public static void registerDelayedRepeatingTask(Runnable task, long delay, long period, TimeUnit unit) {
		if (!_started)
			throw new ThreadManagerException("Thread Manager is not started.");
		manager.scheduleAtFixedRate(task, delay, period, unit);
	}

	public static void registerScheduledRepeatingTask(Runnable task, Date start, long period, TimeUnit unit) {
		// TODO code here
	}
	...
	private static void initCoreTasks() {
		...
		for (CoreTasks task : CoreTasks.values()) {
			try {
				run = (Runnable) Class.forName(task.className).newInstance();
